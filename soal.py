...
def dfine(ujianx, name):
	abc = list("ABCDE")
	ujian, abcd = [], []
	cek = par(r.get("http://103.139.175.10/ujianutama/index.php?content=ujian&ujian=%s" % (ujianx), allow_redirects=False).text, "html.parser")
...
def start():
	...
	for xx in range(1, 2000):
		cek = r.get("http://103.139.175.10/ujianutama/index.php?content=confirm&id=%s" % (xx), allow_redirects=False).text
